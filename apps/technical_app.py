...
def _render_password_gate(message: str) -> None:
    ...
    st.markdown(
        ...
        <section class="safesandesh-password-gate">
          <div class="safesandesh-password-kicker">SafeSandesh Technical</div>
...
def require_technical_password() -> None:
    ...
    if not expected:
        _render_password_gate(
            "Technical access is disabled because no password is configured. "
    ...
    _render_password_gate(
        "Enter the technical access password to open the private dashboard, AI Studio, model comparisons, and stored scan analytics."
    )
    with st.form("technical_password_form"):
        password = st.text_input("Password", type="password")
        submitted = st.form_submit_button("Open Technical App")
